...
def test_update_review_success(client, db_session):
    ...
    response = client.patch(f"/api/reviews/{review.id}", json=update_data, headers=user_headers)
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
    assert data["rating"] == 5
...
def test_update_review_unauthorized(client, db_session):
    ...
    response = client.patch(f"/api/reviews/{review.id}", json=update_data, headers=user2_headers)
