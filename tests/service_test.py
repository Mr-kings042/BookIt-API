...
def test_admin_update_service(client, db_session):
    ...
    response = client.patch(f"/api/services/{service.id}", json=update_data, headers=admin_headers)
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
...
def test_create_service_invalid_data(client, db_session):
    ...
    response = client.post("/api/services", json=invalid_service_data, headers=admin_headers)
...
def test_update_nonexistent_service(client, db_session):
    ...
    response = client.patch(f"/api/services/{nonexistent_id}", json=update_data, headers=admin_headers)
    assert response.status_code == status.HTTP_404_NOT_FOUND
