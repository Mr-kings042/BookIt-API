...
def test_update_booking_reschedule(client, db_session):
    ...
    response = client.patch(f"/api/bookings/{booking.id}", json=update_data, headers=user_headers)
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
    assert data["id"] == str(booking.id)
...
def test_admin_update_booking_status(client, db_session):
    ...
    response = client.patch(f"/api/admin/bookings/{booking.id}/status?status=confirmed", headers=admin_headers)
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
    assert data["status"] == "confirmed"
...
def test_admin_get_service_bookings(client, db_session):
    ...
    response = client.get(f"/api/services/{service1.id}/bookings", headers=admin_headers)
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
    assert len(data) == 1
...
def test_regular_user_cannot_access_admin_endpoints(client, db_session):
    ...
    response = client.patch(f"/api/admin/bookings/{booking_id}/status?status=confirmed", headers=user_headers)
    assert response.status_code == status.HTTP_403_FORBIDDEN
...
def test_update_nonexistent_booking(client, db_session):
    ...
    response = client.patch(f"/api/bookings/{nonexistent_id}", json=update_data, headers=user_headers)
    assert response.status_code == status.HTTP_404_NOT_FOUND
