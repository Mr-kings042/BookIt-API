...
async def user_token(
    ...
    try:
        ...
        logger.error(f"Error during token generation for {form_data.username}: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
# @user_router.post("/users/", response_model=UserOut, status_code=status.HTTP_201_CREATED, deprecated=True)
# def create_user(user: UserCreate, db: Session = Depends(get_db)):
#     """Create user (deprecated - use /auth/register instead)"""
#     try:
#         logger.info(f"Creating user via legacy endpoint: {user.email}")
#         db_user = user_crud.create_user(db, user)
#         logger.info(f"User created via legacy endpoint: {user.email}")
#         return UserOut.model_validate(db_user)
